...
export default function CartScreen() {
  ...
  const handlePlaceOrder = async () => {
    ...
    const fullAddress = [profile?.address, profile?.flat].filter(Boolean).join(", ");

    if (!profile?.phone) {
      Alert.alert(t("cart.missingAddress"), undefined, [
        { text: t("cart.addAddress"), onPress: () => router.push("/address-edit") },
    ...
    >
      ...
      <RNView style={{
        ...
        <Text style={{ fontSize: 18, fontWeight: "700", color: TEXT }}>
          {t("cart.title")}{" "}
          {totalItems > 0 && <Text style={{ color: AMBER }}>({totalItems})</Text>}
        </Text>
        {items.length > 0 ? (
          <TouchableOpacity
            onPress={() => {
            ...
            >
              {items.map((item) => (
                ...
                >
                  ...
                  {/* Info */}
                  <RNView style={{ flex: 1 }}>
                    <Text style={{ fontSize: 11, color: AMBER, fontWeight: "600", marginBottom: 3 }}>
                      {item.categoryTitle}
                    </Text>
                    <Text
                      numberOfLines={2}
                      style={{ fontSize: 13, fontWeight: "600", color: TEXT, lineHeight: 18, marginBottom: 6 }}
                    >
                      {item.name}
                    </Text>
                    <Text style={{ fontSize: 16, fontWeight: "800", color: AMBER }}>
                      {item.price}.{item.dec}{" "}
                      <Text style={{ fontSize: 11, color: SUBTEXT, fontWeight: "400" }}>TL</Text>
                    </Text>
                  </RNView>
                  ...
                  <RNView style={{ alignItems: "center", gap: 8 }}>
                    ...
                    <RNView style={{
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                          updateQty(item.id, item.categoryId, item.quantity - 1);
                        }}
                        style={{ width: 32, height: 32, alignItems: "center", justifyContent: "center" }}
                      >
                        <Ionicons name="remove" size={16} color={TEXT} />
                      </TouchableOpacity>
                      <Text style={{ fontSize: 14, fontWeight: "700", color: TEXT, minWidth: 24, textAlign: "center" }}>
                        {item.quantity}
                      </Text>
                      <TouchableOpacity
                        onPress={() => {
            ...
            <RNView style={{
              ...
              <RNView style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 6 }}>
                <Text style={{ fontSize: 13, color: SUBTEXT }}>
                  {totalItems} {t(totalItems !== 1 ? "cart.items" : "cart.item")}
                </Text>
              ...
              <RNView style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 14 }}>
                <Text style={{ fontSize: 13, color: SUBTEXT }}>
                  {t("cart.delivery")}{" "}
                  <Text style={{ color: "#2ecc71", fontWeight: "700" }}>{t("cart.freeDelivery")}</Text>
                </Text>
              ...
              <TouchableOpacity
                ...
                style={{
                  ...
                  gap: 8, opacity: isPlacingOrder ? 0.7 : 1,
                }}
              >
                {isPlacingOrder ? (
