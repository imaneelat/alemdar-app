...
import { SearchSectionLabel } from "@/components/SearchSectionLabel";
...
const brands = [
  "Arduino", "Espressif", "Raspberry Pi", "Adafruit", "Apple",
  "Dell", "HP", "Huawei", "ASUS", "Intel", "AMD", "Bosch", "Broadcom", "JBL",
];

const brandLogos: Record<string, { path: string; hex: string }> = {
  Arduino: siArduino, Espressif: siEspressif, "Raspberry Pi": siRaspberrypi,
  Adafruit: siAdafruit, Apple: siApple, Dell: siDell, HP: siHp,
  Huawei: siHuawei, ASUS: siAsus, Intel: siIntel, AMD: siAmd,
  Bosch: siBosch, Broadcom: siBroadcom, JBL: siJbl,
};
...
export default function SearchScreen() {
  ...
  const [availability,        setAvailability]        = useState<AvailabilityFilter>("instock");
  ...
  const {
    ...
    isRefetching: searchRefetching, isError: searchError, refetch: refetchSearch,
  ...
  const {
    ...
    hasNextPage: popularHasNext, fetchNextPage: fetchNextPopular,
  ...
  const filteredResults = useMemo(() => {
    ...
    if (selectedCategories.length > 0) {
      list = list.filter((item) =>
        selectedCategories.some((cat) => item.section.toLowerCase() === cat.toLowerCase())
      );
    }
    if (selectedBrands.length > 0) {
      list = list.filter((item) => {
        const name = item.title.toLowerCase();
        return selectedBrands.some((brand) => name.includes(brand.toLowerCase()));
      });
    }
    return list;
  ...
  const openProduct = (item: ApiProduct) => {
    ...
    router.push({
      ...
      params: { productId: String(item.id), section: item.tableKey },
    });
  };
  ...
  const handleCancel  = () => { setQuery(""); inputRef.current?.blur(); };
  const openFilter    = () => { Keyboard.dismiss(); bottomSheetRef.current?.present(); };
  ...
  const handleEndReached = () => {
    if (!isSearching && popularHasNext && !popularFetchingNext) fetchNextPopular();
  };
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: t.bg }} edges={offlineBannerVisible ? [] : ["top"]}>
      ...
      {searchUnavailableOffline ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 32, gap: 16 }}>
          ...
          <Text style={{ color: t.text, fontSize: 16, fontWeight: "700", textAlign: "center" }}>
            {i18nT("offline.searchUnavailable")}
          </Text>
          <TouchableOpacity
            onPress={() => refetchSearch()}
            style={{ backgroundColor: t.accent, borderRadius: 10, paddingHorizontal: 18, paddingVertical: 10 }}
          >
        ...
        <FlashList
          ...
          keyExtractor={(item: any) => `${item.tableKey ?? item.section}-${item.id}`}
          ...
          onRefresh={() => { if (isSearching) void refetchSearch(); }}
                ...
                <View>
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 20 }}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 12 }}>
                      <SearchSectionLabel label={i18nT("search.recentSearches")} subtext={t.subtext} marginBottom={0} />
                  ...
                  <View style={{ paddingHorizontal: 16, marginTop: 28 }}>
                    <SearchSectionLabel label={i18nT("search.popularSearches")} subtext={t.subtext} />
                  ...
                  {/* ── Popular Products ── */}
                  <View style={{ paddingHorizontal: 16, marginTop: 28, marginBottom: 4 }}>
                    <SearchSectionLabel label="Popular Products" subtext={t.subtext} marginBottom={0} />
                  </View>
                  {popularLoading && (
              ...
              >
                ...
                <TouchableOpacity
                  onPress={() => {
                    ...
                    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    toggleWishlist({
                      id: String(item.id), name, price: "0", dec: "00",
                      stock: "In Stock", low: false,
                      sectionId: item.tableKey, sectionTitle: item.section,
                      accentColor: "#f5a623",
                    });
                  }}
      ...
      >
        ...
        >
          ...
          <View style={{ marginBottom: 24 }}>
            <FilterSectionHeader icon="grid-outline" label={i18nT("search.categories")} accent={t.accent} text={t.text} />
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={{ flexDirection: "row", gap: 8 }}>
                ...
                <Chip
                  ...
                  textStyle={{ color: t.chipText, fontSize: 13, fontWeight: "500" }}
                  icon={() => <Ionicons name="chevron-down" size={12} color={t.chipText} />}
                >
                  {i18nT("search.more")}
                </Chip>
              </View>
          ...
          {/* ── Price Range ── */}
          <View style={{ marginBottom: 24 }}>
            <FilterSectionHeader icon="cash-outline" label={i18nT("search.priceRange")} accent={t.accent} text={t.text} marginBottom={6} />
            ...
            <View style={{ alignItems: "center" }}>
              <RangeSlider
                ...
                thumbSize={24} trackHeight={4} minimumDistance={50}
          ...
          {/* ── Availability ── */}
          <View style={{ marginBottom: 24 }}>
            <FilterSectionHeader icon="checkmark-circle-outline" label={i18nT("search.availability")} accent={t.accent} text={t.text} />
            <SegmentedButtons
              value={availability}
          ...
          <View style={{ marginBottom: 32 }}>
            <FilterSectionHeader icon="swap-vertical-outline" label={i18nT("search.sortBy")} accent={t.accent} text={t.text} />
