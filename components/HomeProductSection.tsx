...
import { ActivityIndicator, ScrollView, TouchableOpacity, View as RNView, useColorScheme } from 'react-native';
...
export function HomeProductSection({ sectionKey, limit = 10 }: Props) {
  ...
  const { data, isLoading, isError } = useSectionProducts(sectionKey, { limit });
  ...
  const viewAll = () =>
    router.push({ pathname: '/category-detail', params: { section: sectionKey } });
      ...
      <RNView style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', paddingHorizontal: 16, marginBottom: 4 }}>
        <RNView style={{ flex: 1 }}>
          <RNView style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            <Text style={{ fontSize: 18, fontWeight: '700', color: TEXT }}>{meta.title}</Text>
          </RNView>
          <Text style={{ fontSize: 11, color: SUBTEXT, marginTop: 3, marginLeft: 12 }}>{meta.subtitle}</Text>
        </RNView>
        <TouchableOpacity onPress={viewAll} style={{ flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 2 }}>
          <Text style={{ fontSize: 12, fontWeight: '600', color: meta.accentColor }}>View All</Text>
          <Ionicons name="arrow-forward" size={13} color={meta.accentColor} />
        </TouchableOpacity>
      </RNView>

      {isLoading ? (
        <RNView style={{ height: 200, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 10, paddingBottom: 4 }}>
          {products.map((p) => (
            <ProductCard key={`${p.section}-${p.id}`} product={p} sectionTitle={meta.title} accentColor={meta.accentColor} />
          ))}
        </ScrollView>
      )}
    </RNView>
